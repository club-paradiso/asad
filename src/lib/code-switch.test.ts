...
describe("what the pair can be read at all", () => {
  ...
  it("says so honestly instead of returning a confident nonsense reading", () => {
    ...
    // And reports NO foreign spans, because there is no evidence of one. An
    // ordinary English word in an English↔Spanish session is not a code switch;
    // treating every Latin run as a guest term there would have reported the
    // whole sentence as foreign.
    expect(analysis.guestTerms).toEqual([]);
  });

  it("still protects identifiers when the scripts cannot separate the pair", () => {
    // The script-independent half survives: `GPT-4` and `HTTP 403` are worth
    // preserving in any session, and saying so claims nothing about language.
    const analysis = analyseCodeSwitch("We called GPT-4 and it returned HTTP 403.", {
      source: "en-US",
      target: "es-ES",
    });
    expect(analysis.guestTerms).toEqual(["GPT-4", "HTTP 403"]);
  });
});
...
describe("reading one stabilised unit", () => {
  ...
  it("reads a Korean sentence carrying an English noun phrase", () => {
    ...
    // The PHRASE, not its halves: every consumer wants the whole thing. The
    // prompt asks the model to preserve it, and "social capital" is a better
    // recogniser keyterm than "social" and "capital" separately.
...
describe("terms worth carrying through untouched", () => {
  it("keeps technical phrases, product names and foreign names", () => {
    expect(guestTermsIn("오늘은 retrieval augmented generation, 그러니까 RAG 구조입니다.")).toEqual([
      // One phrase; the comma ends it rather than bridging into the Korean.
  ...
  it("keeps identifiers with their hyphens and digits intact", () => {
    ...
    // The status code travels with the protocol that produced it.
    expect(guestTermsIn("서버가 HTTP 403을 반환했습니다.")).toEqual(["HTTP 403"]);
    expect(guestTermsIn("GPT-4o를 사용했습니다.")).toEqual(["GPT-4o"]);
  });

  it("keeps an apostrophe and a quoted clause intact", () => {
    // The quotation is one span the speaker delivered in another language, and
    // it reaches the prompt as one span. The apostrophe never splits a word.
    expect(guestTermsIn("그가 말했어요. “I don't know.”")).toEqual(["I don't know"]);
  });
