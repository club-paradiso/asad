...
- 놓치기 쉬운 문맥 붙잡기
- 고유명사와 용어 기억하기 (한 번 정해진 이름 표기는 세션 내내 흔들리지 않습니다)
- 성경 구절 참조 잡기
...
#### 시작 전에 고를 것은 언어 한 쌍뿐입니다

예전에는 시작하기 전에 **설교 모드냐 일반 모드냐**를 먼저 정해야 했습니다. 아무도 말을 꺼내기 전에, 나중에 바꿀 수도 없이요.
...
상황(예배·강연·회의·대화·행사)은 **ASAD가 말에서 읽어냅니다.** 성경 구절이 잡히는지, 어떤 용어가 얼마나 자주 나오는지, 어떤 담화 표현이 쓰이는지, 준비 시트에 뭐라고 적혀 있는지, 그리고 어차피 보내던 통역 요청에 모델이 얹어 보내주는 판단까지 — 여섯 갈래 신호를 합쳐서 정합니다. **상황 판단 때문에 모델을 한 번 더 부르지는 않습니다.**

콘솔 위쪽에 `Auto · Worship` 처럼 지금 무엇으로 보고 있는지 떠 있고, 마음에 안 들면 거기서 한 번에 바꿀 수 있습니다. 바꿀 일이 없는 게 정상입니다.

한 문장 잘못 나왔다고 상황이 휙휙 바뀌지는 않습니다. 새 후보가 기존 판단을 일정 점수 이상 앞질러야만 넘어갑니다. 지나가는 말 한마디로 예배가 회의가 되면 그게 더 이상한 일이니까요.
...
- **어느 쪽이 끝내든 양쪽 화면이 알아서 빠져나갑니다.** 방문자 화면은 종료 안내로 넘어가고(브라우저가 허용하면 탭까지 닫습니다), 직원 화면은 시작 화면으로 돌아갑니다. 끝난 대화를 그대로 띄워둔 기기만큼 위험한 물건이 없습니다. 다음 손님은 새 QR로 새로 시작하면 됩니다.
...
| `/prep` | 화자, 본문, 용어 등을 미리 넣어 AI에게 예습을 시킵니다 |
...
언어에 대해 아는 것은 **`src/lib/languages.ts` 한 곳**에 모여 있습니다. 태그, 문자 체계, 쓰기 방향, 띄어쓰기 여부, 인식기별 지원 수준, 크롬 기기 내 번역기 코드, 그리고 그 언어를 어디에 내놓아도 되는지까지 전부요.

예전에는 이 정보가 다섯 군데에 흩어져 있었고, 그래서 Counter에는 25개 언어가 있는데 Live는 한국어→영어 하나뿐이었습니다. 이제 **Live도 같은 목록에서 언어 쌍을 고릅니다.**
...
`zh-CN`과 `zh-TW`는 읽는 사람 입장에서 서로 다른 글입니다. 그런데 Whisper 계열 음성 인식 API는 **언어는 고르게 해주지만 문자 체계는 고르게 해주지 않습니다.** `zh`만 받습니다.

그래서 번체를 고른 방문자가 간체로 받아적혀 있는데도 시스템은 "성공"이라고 보고하는 일이 생깁니다.

지금은 이렇게 처리합니다.

1. 레지스트리가 그 한계를 `variant-lossy`라고 **명시**합니다. 숨기지 않습니다.
2. 문자 체계를 못 지키는 경로는, 지킬 수 있는 경로보다 **뒤로 밀립니다.** OpenAI만 설정된 배포에서 번체를 고르면 브라우저 인식으로 먼저 갑니다.
3. 그래도 그 경로밖에 없으면 **화면에 그렇게 적힙니다.** 조용히 넘어가지 않습니다.
4. 번역 쪽에도 목표 문자 체계를 프롬프트에 못박습니다. "Chinese"라고만 하면 모델은 간체를 내놓으니까요.

이건 중국어 전용 땜질이 아닙니다. 원인이 "태그가 담고 있는 문자 정보를 공급자 API가 표현하지 못한다"이기 때문에, 같은 모양의 다른 언어 쌍도 같은 규칙으로 처리됩니다.
...
언어별로 어떤 인식기가 실제로 그 언어를 처리할 수 있는지는 `src/providers/stt/capability.ts`에 선언되어 있습니다. 처리할 수 없는 인식기에는 아예 연결을 시도하지 않습니다.

**위구르어는 현재 타이핑 전용입니다.** 이 스택에 설정된 어떤 인식기도 위구르어를 받지 않기 때문에, 마이크 버튼을 보여 주고 실패하게 두는 대신 처음부터 텍스트 입력을 정상 경로로 안내합니다. 위구르어는 아랍 문자를 쓰지만 아랍어가 아니고, 튀르크어족이지만 우즈베크어나 터키어도 아닙니다. 그 셋 중 어느 쪽으로도 라우팅되지 않습니다.
...
| [`docs/context-intelligence.md`](docs/context-intelligence.md) | 상황 자동 인식 · 언어 레지스트리 |
| [`docs/live-upgrade-audit.md`](docs/live-upgrade-audit.md) | 이번 개편 전 감사 기록 |
| [`docs/sermon-booth.md`](docs/sermon-booth.md) | 예배 통역/부스 운영 |
